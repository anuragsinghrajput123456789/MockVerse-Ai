...
export function useExport(activeSheet: ResourceSheet | null) {
  ...
  const handleExportPdf = useCallback(async () => {
    ...
    try {
      toast({
        title: "Compiling PDF...",
        description: "Your study study sheet PDF is generating on the server.",
      });
  ...
  const handleExportHtml = useCallback(async () => {
    ...
    try {
      toast({
        ...
        description: "Your study study sheet HTML is generating on the server.",
      });
      const sheetId = activeSheet.id || (activeSheet as any)._id;
